# =============================================================================
# Quick Push Script for Interview Microservice ABC
# =============================================================================
# Description: Build and push all microservices to Docker Hub
# Usage: .\quick-push.ps1 [options]
# Options:
...
$Services = @{
    ...
    # "nlp-service" = @{Image = "interview-nlp-service"; Description = "NLP Processing Service"} # Disabled
}
...
function Test-DockerLogin {
    try {
        # Check if we can access Docker Hub by checking config
        ...
        # If config check fails, assume logged in if docker command works
        try {
            docker ps | Out-Null
            return $true
        } catch {
...
function Show-PushSummary {
    ...
    Write-Host "  • Update docker-compose.yml to use Docker Hub images" -ForegroundColor White
    Write-Host "  • Run '.\quick-deploy.ps1' to test deployment" -ForegroundColor White
    Write-Host "  • Share the Docker Hub URLs with your team" -ForegroundColor White
}

function Show-Help {
    Write-Header "Help - $ScriptName v$Version" "Blue"
    Write-Host "`nUsage: .\quick-push.ps1 [options]" -ForegroundColor Yellow
    ...
    Write-Host "  .\quick-push.ps1                           # Push all services with 'latest' tag" -ForegroundColor White
    Write-Host "  .\quick-push.ps1 -Tag v1.0.0              # Push all services with 'v1.0.0' tag" -ForegroundColor White
    Write-Host "  .\quick-push.ps1 -Service auth            # Push only auth-service" -ForegroundColor White
    Write-Host "  .\quick-push.ps1 -NoBuild -Tag v1.0.0     # Push existing images with custom tag" -ForegroundColor White
    Write-Host "  .\quick-push.ps1 -Verbose -NoCache        # Verbose build and push without cache" -ForegroundColor White
...
if ($pushSuccess) {
    ...
    Write-Host "For help, run: .\quick-push.ps1 -Help" -ForegroundColor Cyan
    exit 1
}
